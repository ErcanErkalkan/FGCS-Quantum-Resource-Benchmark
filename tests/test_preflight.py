...
def test_uw_claim_fence_manifest_flags_must_be_false(tmp_path):
    root = _mini_project(tmp_path)
    ...
    (root / 'manuscript' / 'main.tex').write_text(
        ...
        'not consistently logged at the exact model-version level',
        encoding='utf-8',
    )
    ...
    ok = {'hardware_claims': False, 'compiler_claims': False}
    assert all(c.status == 'PASS' for c in check_claim_fences(root, ok))
    bad = {'hardware_claims': True, 'compiler_claims': False}
...
def test_p1_preflight_rejects_legacy_random_family_label(tmp_path):
    ...
    checks = check_claim_fences(root, {'hardware_claims': False, 'compiler_claims': False})
